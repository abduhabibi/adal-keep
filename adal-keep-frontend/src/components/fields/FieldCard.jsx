...
export default function FieldCard({ field, profileId, onFileUpdate }) {
  ...
  // Counter to prevent flickering when dragging over child elements
  ...
  const handleDrop = (e) => {
    ...
    // 1. Check for External File (from Desktop/Finder)
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      fileToUpload = e.dataTransfer.files[0]
    }
    // 2. Check for Internal Tray File (via ID)
    else if (e.dataTransfer.getData('application/x-tray-file-id')) {
      const fileId = e.dataTransfer.getData('application/x-tray-file-id')
      fileToUpload = capturedFiles.find(f => f.id === fileId)
    }
    // 3. Fallback to Global Window Reference (set by FileTray)
    else if (window.__DRAGGED_TRAY_FILE__) {
      fileToUpload = window.__DRAGGED_TRAY_FILE__
    }

    if (fileToUpload) {
      handleFileSelect(fileToUpload)
    } else {
      // Only show error if it wasn't a text drag (e.g. dragging text selection)
  ...
  return (
    <div
      className={`panel p-4 flex flex-col gap-3 relative bg-white dark:bg-slate-800 border rounded-xl shadow-sm transition-all duration-200 ${
        isDragOver
          ? 'border-teal-500 ring-2 ring-teal-500/30 bg-teal-50/50 dark:bg-teal-900/10 scale-[1.02]'
    ...
    >
      ...
      {field.files?.length > 0 ? (
            ...
            {field.files.map(f => (
              <div key={f.id} className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700 group">
                <div className="flex items-center gap-2 overflow-hidden flex-1 min-w-0">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-300 truncate font-medium" title={f.original_name}>{f.original_name}</span>
                </div>
                <div className="flex gap-1 shrink-0 ml-2">
                  <a href={`/api/files/${f.id}/download`} target="_blank" rel="noopener noreferrer" className="p-1.5 text-slate-400 hover:text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-900/30 rounded-md transition-colors" title="Download">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                  </a>
                  <button type="button" onClick={() => handleDelete(f.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors" title="Delete">
        ...
        <div className={`flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed bg-slate-50/50 dark:bg-slate-900/50 py-6 text-center transition-all duration-200 ${
          isDragOver ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-teal-400 hover:bg-teal-50/50 dark:hover:bg-teal-900/10'
        }`}>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-700">
            ...
            <div className="p-5 space-y-3">
              <p className="text-sm text-slate-600 dark:text-slate-300 text-center">We will save both the original and the converted version.</p>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => handleFormatChoice('image')} className="flex flex-col items-center gap-2 p-4 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800 rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors">
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  <span className="font-semibold text-sm">Image</span>
                </button>
                <button onClick={() => handleFormatChoice('pdf')} className="flex flex-col items-center gap-2 p-4 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-xl hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex justify-end">
              <button onClick={() => setPendingFile(null)} className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-700">
            ...
            <div className="p-4 space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
              {downloadFiles.map(f => (
                <a key={f.id} href={`/api/files/${f.id}/download`} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-teal-50 dark:hover:bg-teal-900/20 hover:border-teal-300 dark:hover:border-teal-800 transition-colors group">
